...
TAU_MAX = 1024   # close enough to int(44100/50)
...
def read_head():
    ...
    for k in range(len(read_indexes)):
        ...
        i += 1
        j += shift
        if shift > 1 and j-i >= 1:
            i += 1
        elif shift < 1 and i-j >= 1:
            i -= 1
    read_indexes += int(read_pos)
